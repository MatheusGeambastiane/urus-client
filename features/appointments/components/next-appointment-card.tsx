...
export const NextAppointmentCard = ({
  ...
  const [actionLoading, setActionLoading] = useState<"edit" | "cancel" | null>(
    null
  );
  ...
  const getDraftTime = () => {
    const parsed = new Date(activeAppointment.date_time);
  ...
  const handleEdit = (event: MouseEvent<HTMLButtonElement>) => {
    ...
    const dateTime = new Date(activeAppointment.date_time);
    const draft: AppointmentDraft = {
      appointmentId: activeAppointment.id,
      serviceId,
      serviceName: activeAppointment.service_name,
      servicePrice: activeAppointment.price_paid,
      date: formatDateKey(dateTime),
      time: getDraftTime(),
      professionalId: activeAppointment.professional,
      professionalName: activeAppointment.professional_name,
    };
    saveAppointmentDraft(draft);
    setActionLoading("edit");
    router.push(
      `/services/${serviceId}/schedule?edit_appointment=${activeAppointment.id}`
    );
  };
